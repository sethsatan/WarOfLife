...
def main():     
    ...
    with tf.name_scope("model"):
        px = NY*(NX//3)
        x = tf.placeholder(tf.float32, shape=[None, px])
        
        py_= ((NY*NX)//(5*5))*8                  
        y_ = tf.placeholder(tf.float32, shape=[None, py_])

        W = tf.Variable(tf.zeros([px,py_]))
        b = tf.Variable(tf.zeros([py_]))
    ...
    with tf.name_scope("train"):
        ...
        red_point_ = tf.zeros([py_])
        green_point_ = tf.zeros([py_])
        blue_point_ = tf.zeros([py_])
                
        red_point = tf.count_nonzero(r_pre_neighbours)
        green_point = tf.count_nonzero(g_pre_neighbours)
        blue_point = tf.count_nonzero(b_pre_neighbours)
    ...
    with tf.name_scope("session"):
        ...
        for step in range(n_step):
            ...
            while True:
                ...
                if (gen==10):
                    red_point = sess.run(red_point)
                    green_point = sess.run(green_point)
                    blue_point = sess.run(blue_point)
                    if (red_point< py_):
                    ...
                    sess.run(feed_dict={x: state0, y_: red_point_})
                    sess.run(train_step)
                    sess.run(feed_dict={x: state1, y_: green_point_})
                    sess.run(train_step)
                    sess.run(feed_dict={x: state2, y_: blue_point_})
                    sess.run(train_step)
                    break
